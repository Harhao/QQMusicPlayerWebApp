      ...
      <div id="lyrics">
        ...
        <div id="lyricsCon" class="hide" @click="togglelyricsCon">
            <div id="custom" @touchstart="start" @touchmove="move" @touchend="end">
              <p v-for="(item,index) in lyric" :data-index="index" :key="index">
                <span v-for="(row,idx) in item" :data-time="idx" :key="idx">{{row}}</span>
              </p>
            </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    lyric() {
      let globalArr = [];
      this.$http
        .get("http://localhost:3000/lyrics?id=" + this.songPlayList[0].mid)
        .then(response => {
  ...
  methods: {
    ...
    updateCurrentTime() {
      ...
      var percent =
        audio.currentTime.toFixed(0) / audio.duration.toFixed(0) * 100;
    ...
    move(event) {
      ...
      document.getElementById("custom").style.top =
        parseFloat(top) + distance + "px";
    },
    end(event) {
...
#musicPlayer {
  ...
  animation: changeIn 0.3s ease-in-out;
}
#back {
  width: 100%;
  height: 60px;
