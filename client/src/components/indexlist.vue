...
export default {
  ...
  created() {
    ...
    this.$http
      .get("http://localhost:3000/indexlist?id=" + this.id)
      .then(response => {
  ...
  methods: {
    scrollFixed(e) {
      ...
      if (e.target.scrollTop > height) {
        ...
        this.$http
          .get(
            "http://localhost:3000/indexlist?id=" +
              this.id +
              "&curIndex=" +
              this.index
          )
...
.listData li:nth-child(1) .index,
.listData li:nth-child(2) .index,
.listData li:nth-child(3) .index {
  color: #ff400b;
}
.listData li {
  width: 100%;
